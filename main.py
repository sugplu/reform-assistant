...
2. Uploaded knowledge files in the knowledge folder.
...
def build_system_prompt() -> str:
    ...
    if EXTRA_KNOWLEDGE:
        prompt += (
            "\n\n━━━ KNOWLEDGE FILES ━━━\n"
            "Use these as an internal reference for FAQs, templates, operational details, "
            "and approved messaging when relevant.\n\n"
            f"{EXTRA_KNOWLEDGE}"
        )
...
async def health():
    ...
    return {
        "status": "ok",
        "knowledge_loaded": bool(EXTRA_KNOWLEDGE),
        "knowledge_chars": len(EXTRA_KNOWLEDGE),
    ...
    def generate():
        try:
            stream = client.chat.completions.create(
                ...
                stream=True,
                max_completion_tokens=int(os.getenv("MAX_TOKENS", "900")),
